package me.isach.musicalsheeps.arena;
...
import me.isach.musicalsheeps.entity.PathfinderGoalPanicSheep;
import me.isach.musicalsheeps.listeners.SignsManager;
import me.isach.musicalsheeps.util.*;
...
public class Arena {
     ...
     * Arena Constructor
     *
     * @param id The {@see me.isach.musicalsheeps.arena.Arena} ID.
     */
    ...
    public void addPlayer(Player player) {
        ...
        File f = new File(Core.getPlugin().getDataFolder().getPath() + "/songs/" + getSong() + ".nbs");
        if (!f.exists()) {
    ...
    public void start() {
        ...
        countdown.runTaskTimer(Core.getPlugin(), 0, 20);
    }
    ...
    private class Countdown extends BukkitRunnable {
        ...
        public void run() {
            ...
            if (countingNums.contains(timer)) {
                if (timer <= 5)
                    for (PlayerData pd : data)
                        pd.getPlayer().playSound(pd.getPlayer().getLocation(), Sound.NOTE_STICKS, 1, 1);
    ...
    public class Round extends BukkitRunnable {
        ...
        public void run() {
            ...
            for (int i = 0; i < sheepsAmount; i++) {
                ...
                CrazySheep ces = new CrazySheep(nmsWorld);
                ces.setPosition(center.getX(), center.getY(), center.getZ());
                nmsWorld.addEntity(ces);
                ...
                entitySheep.goalSelector.a(3, new PathfinderGoalPanicSheep(entitySheep, 0.4d));
            ...
            File f = new File("/plugins/MusicalSheeps/songs/");
            if (!f.exists())
                f.mkdirs();
            Song s = NBSDecoder.parse(new File(Core.getPlugin().getDataFolder(), "songs/" + getSong() + ".nbs"));
            ...
            final int A = Bukkit.getScheduler().runTaskTimer(Core.getPlugin(), new Runnable() {
                @Override
                public void run() {
                    for (Sheep s : sheeps) {
                        s.getWorld().spigot().playEffect(s.getLocation(), Effect.NOTE, 0, MathsUtils.randInt(1, 24), 1.3f, 0.6f, 1.3f, .1f, 5, 32);
                        s.setColor(DyeColor.getByData((byte) MathsUtils.randInt(1, 15)));
                    }
                }
            }, 0, 3).getTaskId();
            Bukkit.getScheduler().runTaskLater(Core.getPlugin(), new Runnable() {
                ...
                public void run() {
                    ...
                    for (Player players : getPlayers()) {
                        players.playSound(players.getLocation(), Sound.VILLAGER_NO, 3, 1);
                        Title t = new Title("§4");
                        t.setSubtitle(MessageManager.getMessage("Get-On-A-Sheep"));
                        t.send(players);
                    }
                    sp.setPlaying(false);
                    final int B = Bukkit.getScheduler().runTaskTimer(Core.getPlugin(), new Runnable() {
                        double i = 10;

                        @Override
                        public void run() {
                            for (Player p : getPlayers()) {
                                ActionBarAPI.send(p, MessageManager.getMessage("Elimination-In").replaceAll("%t%", new DecimalFormat("00.0").format(i) + ""));
                            }
                            i -= 0.1;
                        }
                    }, 0, 2).getTaskId();
                    Bukkit.getScheduler().runTaskLater(Core.getPlugin(), new Runnable() {
                        ...
                        public void run() {
                            ...
                            for (Player p : getPlayers())
                                ActionBarAPI.send(p, MessageManager.getMessage("Eliminating"));
                            for (Player p : getPlayers()) {
                            ...
                            for (Sheep sheep : sheeps) {
                                getWorld().spigot().playEffect(sheep.getLocation(), Effect.LAVA_POP, 0, 0, 0, 0, 0, 0, 10, 32);
                                sheep.remove();
                            }
                            cancel();
                            if (currentPlayers > 1) {
                                ...
                                currentRound++;
                                for (Player p : getPlayers())
                                    new Title(MessageManager.getMessage("Round").replaceAll("%round%", currentRound + "")).send(p);
                                round.runTask(Core.getPlugin());
                            }
                        }
                    }, 10 * 20);
